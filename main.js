...
countries.forEach(country => {         
    ...
    arr = countries.filter(data => {         return data.toLowerCase().startsWith(searchWord);     }).map(data => {         let isSelected = data === selectBtn.firstElementChild.innerText ? "selected" : "";         let countryLink = countryLinks[data];         return `<li onclick="updateName(this); window.location='${countryLink}'" class="${isSelected}">${data}</li>`;     }).join("");     options.innerHTML = arr ? arr : '<p style="margin-top: 10px;">Oops! State not found</p>'; });  selectBtn.addEventListener("click", () => wrapper.classList.toggle("active"));